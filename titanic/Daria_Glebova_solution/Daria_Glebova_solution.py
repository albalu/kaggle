...
gender=pd.read_csv(os.path.join(dir, '../data/gender_submission.csv'))
train=pd.read_csv(os.path.join(dir, '../data/train.csv'))
test=pd.read_csv(os.path.join(dir, '../data/test.csv'))

logging.info(train.isnull().sum())

logging.info(gender.isnull().values.any())
...
logging.info(fc)
logging.info(fc_test)

# fill out the missing Fare data according to their class average for both train and test data:
...
index = test['Fare'].index[test['Fare'].apply(np.isnan)] # there is one person with unknown Fare left in the test data
test['Fare'].values[index.values[0]] = fc_test['Fare'].values[2] # I still don't know why she picked 2 here!?
...
submit.to_csv('Daria_submit_prediction.csv', index=False)
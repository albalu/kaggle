...
# logging.basicConfig(level=logging.DEBUG, format='%(message)s') # format='%(message)s' is to avoid "ROOT: INFO", etc
...
if __name__ == "__main__":
    ...
    #-----------------------------------------------------------
    train_df = pd.read_csv(os.path.join(dir, '../data/train.csv'))  # train set
    test_df  = pd.read_csv(os.path.join(dir, '../data/test.csv'))   # test  set
    ...
    #-----------------------------------------------------------
    # logging.debug(train_df.head(5))
    # logging.debug(train_df.info()) # valuable information regarding incomplete features or type of data, etc
    logging.debug(train_df.describe())
    logging.debug(train_df.describe(include=['O'])) # include Object data types
    ...
    temp = train_df[['Pclass', 'Survived']].groupby(['Pclass'], as_index=False).mean()
    logging.info(temp.sort_values(by='Survived', ascending=False))
    ...
    logging.info(train_df[['Title', 'Survived']].groupby(['Title'], as_index=False).mean().sort_values(
    ...
    logging.info(train_df[['AgeBand', 'Survived']].groupby(['AgeBand'], as_index=False).mean().sort_values(
    ...
    logging.info(train_df[['FamilySize', 'Survived']].groupby(['FamilySize'], as_index=False).mean().sort_values(
            by='Survived', ascending=False))
    ...
    logging.info(train_df[['IsAlone','Survived']].groupby(['IsAlone'], as_index=False).mean())
    ...
    logging.debug(train_df.loc[:, ['Age*Class', 'Age', 'Pclass']].head())

    freq_port = train_df.Embarked.dropna().mode()[0]
    for dataset in combine:
        dataset['Embarked'] = dataset['Embarked'].fillna(freq_port)
    logging.info(train_df[['Embarked', 'Survived']].groupby(['Embarked'], as_index=False).mean().sort_values(
            by='Survived', ascending=False))
    ...
    # fill the missing values of Fare, NOTE that in this particular example, train data is NOT missing any Fare info
    test_df['Fare'].fillna(test_df['Fare'].dropna().median(), inplace=True)

    # I added this just to confirm that there are no more NA remaining
    # logging.debug(train_df.info())
    # logging.debug(test_df.info())

    # Create FareBand
    train_df['FareBand'] = pd.qcut(train_df['Fare'], 4)
    logging.info(train_df[['FareBand', 'Survived']].groupby(['FareBand'], as_index=False).mean().sort_values(
    ...
    if plot_vc == 1:
        ...
        param_name = "C"
        ylim = [0.55, 0.9]
        plt1 = validation_curve_model(X_data, Y_data, model, "C", param_range, cv, ylim)
        plt1.show()
    ...
    if plot_lc == 1:
        ...
        plt = learning_curve_model(X_data, Y_data, logreg, cv, train_size)
        plt.show()
    ...
    if plot_vc == 1:
        ...
        param_name = "C"
        ylim = [0.78, 0.90]
        plt1 = validation_curve_model(X_data, Y_data, SVC(), "C", param_range, cv, ylim, log=False)
        plt1.show()
    ...
    if plot_lc == 1:
        train_size = np.linspace(.1, 1.0, 15)
        plt2 = learning_curve_model(X_data, Y_data, svc, cv, train_size)
        plt.show()
    ...
    if plot_vc == 1:
        ...
        ylim = [0.75, 0.90]
        plt1 = validation_curve_model(X_data, Y_data, KNeighborsClassifier(), "n_neighbors", param_range, cv, ylim, log=False)
        plt1.show()
    ...
    if plot_lc == 1:
        train_size = np.linspace(.1, 1.0, 15)
        plt2 = learning_curve_model(X_data, Y_data, knn, cv, train_size)
        plt2.show()
    # KNN
    ...
    if plot_vc == 1:
        param_range = np.linspace(10, 110, 10).astype(int)
        ylim = [0.75, 0.90]
        plt1 = validation_curve_model(X_data, Y_data, RandomForestClassifier(min_samples_leaf=12), "n_estimators", param_range,
                               cv, ylim, log=False)
        plt1.shot()
    ...
    if plot_vc == 1:
        ...
        ylim = [0.75, 0.90]
        plt2 = validation_curve_model(X_data, Y_data, RandomForestClassifier(n_estimators=80), "min_samples_leaf", param_range,
                               cv, ylim, log=False)
        plt2.show()
    ...
    # ensemble votring
    ensemble_voting = VotingClassifier(estimators=[('lg', logreg), ('sv', svc), ('rf', random_forest), ('kn', knn)],
                                       voting='soft')
    acc_ensemble_voting = predict_model(X_data, Y_data, ensemble_voting, X_test_kaggle,
    ...
    models = pd.DataFrame({'Model': ['Support Vector Machines', 'KNN', 'Logistic Regression',
                           ...
                           'KFoldScore': [acc_svc.mean(), acc_knn.mean(), acc_log.mean(),
                           ...
                           'Std': [acc_svc.std(), acc_knn.std(), acc_log.std(),
                                   ...
                                   acc_sgd.std(), acc_linear_svc.std(), acc_decision_tree.std(),
                                   acc_ensemble_voting.std()]})

    print(models.sort_values(by='KFoldScore', ascending=False))